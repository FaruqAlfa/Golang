...
func (s *studentRepoImpl) FetchAll() ([]model.Student, error) {
	rows, err := s.db.Query("SELECT id, name, address, class FROM students")//membuat query
	if err != nil {
		return nil, err
	}
	defer rows.Close()//menutup query setiap kali dijalankan

	var students []model.Student //membuat slice student dengan tipe model.Student untuk menampung data
	for rows.Next() {
