...
func (t Teacher) Join(db *gorm.DB) ([]Joined, error) {
	...
	if err := db.Table("teachers").Select("teachers.name as teacher_name, schools.name as school_name, classes.name as class_name, lessons.name as lesson_name").
		Joins("left join schools on teachers.school_id = schools.id").
		Joins("left join classes on teachers.class_id = classes.id").
		Joins("left join lessons on teachers.lesson_id = lessons.id").
